...
// In case of year do not entered during 5 sec returns -1
...
void setup() {
  ...
  // Set the RTC to the PC time at scetch upload or set time via serial interface
  if (rtc.lostPower()) {
    rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
    } else {
      Serial.println("-- Date and Time Setup --");  // set parameters via serial interface
      ...
      if (year >= 0) {
        ...
        rtc.adjust(DateTime(year, month, date, hour, minute, sec));
